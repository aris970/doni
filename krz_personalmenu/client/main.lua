...
local PersonalMenu = {
	...
	WeaponData = {},
	WalletIndex = {},
	WalletList = {_U('wallet_option_give')},
	BillData = {},
...
Citizen.CreateThread(function()
	...
	RMenu.Add('rageui', 'personal', RageUI.CreateMenu(Config.MenuTitle, _U('mainmenu_subtitle'), 1355, 255, 'commonmenu', 'gradient_bgd', 255, 255, 255, 255))
   	RMenu.Add('personal', 'admin', RageUI.CreateSubMenu(RMenu.Get('rageui', 'personal'), _U('admin_title')), function()
		if Player.group ~= nil and (Player.group == 'm2od' or Player.group == 'admi2n' or Player.group == 'superadmin2' or Player.group == 'ow2ner' or Player.group == '_dev') then
			return true
		end
...
RegisterNetEvent('reload_death:onPlayerDeath', function()

	Player.isDead = true

end)

RegisterNetEvent('reload_death:onPlayerRevive', function()
...
RageUI.List(_U('wallet_money_button', ESX.Math.GroupDigits(ESX.PlayerData.money)), PersonalMenu.WalletList, PersonalMenu.WalletIndex['money'] or 1, nil, {}, true, function(Hovered, Active, Selected, Index)
					...
					if post then
						...
						if closestDistance ~= -1 and closestDistance <= 3 then
							...
							if not IsPedSittingInAnyVehicle(closestPed) then
								...
								RageUI.CloseAll()
					         ExecuteCommand("imdekrmkk ~y~Hands over an cash")
							else
		...
		for i = 1, #ESX.PlayerData.accounts, 1 do
			if ESX.PlayerData.accounts[i].name == 'black_money' then
				...
				RageUI.List(_U('wallet_blackmoney_button', ESX.Math.Round(ESX.PlayerData.accounts[i].money)), PersonalMenu.WalletList, PersonalMenu.WalletIndex[ESX.PlayerData.accounts[i].name] or 1, nil, {}, true, function(Hovered, Active, Selected, Index)
					if (Selected) then
						if Index == 1 then
							local post, quantity = CheckQuantity(KeyboardInput('KORIOZ_BOX_AMOUNT', _U('dialogbox_amount'), '', 8))
...
ESX.UI.Menu.CloseAll()
										...
										RageUI.CloseAll()
										ExecuteCommand("imdekrmkk ~y~hands over an item")
									else
										ESX.ShowNotification(_U('in_vehicle_give', 'de l\'argent'))
									end
								else
		...
		for i = 1, #RMenu['personal'], 1 do
			...
			if type(RMenu['personal'][i].Restriction) == 'function' then
				if RMenu['personal'][i].Restriction() then
					RageUI.Button(RMenu['personal'][i].Menu.Title, nil, {RightLabel = nil}, true, function() end, RMenu['personal'][i].Menu)
				else
					RageUI.Button(RMenu['personal'][i].Menu.Title, nil, {RightBadge = nil}, false, function() end, RMenu['personal'][i].Menu)
				end
			else
				...
				if (Selected) then
					...
					if closestDistance ~= -1 and closestDistance <= 3.0 then
						TriggerServerEvent('jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(closestPlayer))
                                                ExecuteCommand("imdekrmkk ~y~Jep leternjoftimin")

					else
						ESX.ShowNotification(_U('players_nearby'))
                                                ExecuteCommand( "me Jep leternjoftimin" )
					end
				end
			end)
...
ESX.UI.Menu.CloseAll()
								...
								RageUI.CloseAll()
								ExecuteCommand("imdekrmkk ~y~Hands over an weapon")
							else
								ESX.ShowNotification(_U('in_vehicle_give', PersonalMenu.ItemSelected.label))
							end
						else
	...
	RageUI.DrawContent({header = true, instructionalButton = true}, function()
		for i = 1, #ESX.PlayerData.inventory, 1 do
			if ESX.PlayerData.inventory[i].count > 0 then
				...
				RageUI.List(ESX.PlayerData.inventory[i].label .. ' (x' .. ESX.PlayerData.inventory[i].count .. ')', invCount, PersonalMenu.ItemIndex[ESX.PlayerData.inventory[i].name] or 1, nil, {}, true, function(Hovered, Active, Selected, Index)
					if (Selected) then
ESX.UI.Menu.CloseAll()
	...
	RageUI.DrawContent({header = true, instructionalButton = true}, function()
		for i = 1, #PersonalMenu.WeaponData, 1 do
			if HasPedGotWeapon(plyPed, PersonalMenu.WeaponData[i].hash, false) then
				...
				RageUI.Button(PersonalMenu.WeaponData[i].label, nil, {RightLabel = ">>>"}, true, function(Hovered, Active, Selected)
					if (Selected) then
ESX.UI.Menu.CloseAll()
...
function RenderAnimationMenu()
	RageUI.DrawContent({header = true, instructionalButton = true}, function()
		for i = 1, #RMenu['animation'], 1 do
			RageUI.Button(RMenu['animation'][i].Menu.Title, nil, {RightLabel = nil}, true, function() end, RMenu['animation'][i].Menu)
		end
	end)
end
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		if IsControlJustPressed(0, Config.Controls.OpenMenu.keyboard) and not Player.isDead and not exports['TRUNK']:IsTrunkOpen() then
